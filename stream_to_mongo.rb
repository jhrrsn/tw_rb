...
############################################
# Define stopwords and punctuation for NLP #
############################################
...
TweetStream::Client.new.locations(-7.9, 49.6, 2.2, 61.1) do |status|
  ...
  entry = {"_id" => status.id.to_i, "time" => Time.now.to_i, "coords" => [status.geo.lat, status.geo.long], "text" => status.text, "name" => status.from_user, "keywords" => split_tweet}
  @coll.insert(entry)
end